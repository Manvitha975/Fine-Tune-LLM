...
#### Mac
- M1/M2/M3 chip with 16GB RAM (Recommended).
...
Install the GPU-enabled version of PyTorch manually:
...
gr.ChatInterface(
    ask_medical_bot,
    title="🏥 Medical AI (Windows)"
).launch()
```

---

# Option B: Mac (Apple Silicon M1/M2/M3)
...
|---|---|---|
| `OSError: [Errno 28] No space left on device` | Win/Mac | Delete `%userprofile%\.cache\huggingface` to free up space |
| `ValueError: Some modules dispatched on CPU/disk...` | Windows | Enable offloading using Option A |
| `KeyError: 'base_model.model.model...'` | Windows | Add `offload_folder="./offload_folder"` |
| `AssertionError: Torch not compiled with CUDA` | Windows | Install GPU-enabled PyTorch |
| App crashes silently / 0% Load | Windows | Increase Virtual Memory (Pagefile) to 25GB |
| `mps device not found` | Mac | Ensure you're using an M-Series Mac with macOS 12+ |
